...
>
> The ADRs are the source of truth for the *why*. This file is the structured *what*. Where they disagree, the ADR wins. · **Last build log entry:** Sprint 13 (E2.1 — session fork & save management, S-2.1.1–S-2.1.3).
...
- **Deferred.** The handoff producer (S-4.2.1 — **now built**, see Sprint 15 below); player input commit (S-5.1.1); the resume anchor (S-5.3.1); the Play advance UI (S-5.4.1); the `npc_moment` branch + recorder-first sequencing (Phase 2); boundary subsystems (Phase 4).
...
The **handoff producer** PH-37 reserved: one narrator turn that runs a structured prose call and feeds its validated `Handoff` into the spine (S-3.1.1). Built backend + tests only — the reachable advance control + prose reader are the Play reader (S-5.4.1, PH-41), so the endpoint exists but nothing in the UI calls it yet. S-4.2.1 is the success path; S-4.2.2 is the safety guarantee — malformed output is retried then surfaced, never trusted.
...
- **Deferred.** Persisting prose/handoff into the `events` scene log (S-5.2.1, PH-40); `elapsed_bucket` consumption by decay (Phase 5); the resume anchor (S-5.3.1); the reachable advance control + prose reader (S-5.4.1, PH-41); the recorder sub-call + `npc_moment` (Phase 2).
