...
>
> The ADRs are the source of truth for the *why*. This file is the structured *what*. Where they disagree, the ADR wins. · **Last build log entry:** Sprint 16 (E5 — Writing/Play reader + scene log + player moment, S-5.4.1 / S-5.2.1 / S-5.1.1).
...
The narrator turn is **two calls** — a prose call (structured output: prose · handoff · inferred elapsed bucket) and a separate **recorder** sub-call (§6). The narrator sees the **beat doc (full)** and the **full mesh** — the NPC never does. The **prose call is now built** (S-4.2.1/S-4.2.2 — see the Build status); the recorder sub-call lands in Phase 2.
...
This inventory is now **backed by data**: the [`prompt_blocks` registry](../adr/0020-prompt-block-registry.md) (ADR 0020) defines each block's purpose, source producers, fold instruction, order, and the **leak rules** that gate it — the assembler reads it to build the prompt, and the human block reference renders from the same rows (one definition, two surfaces). The **narrator half** of that assembler is built ([`NarratorPromptAssembler`](../../app/Services/Narrator/NarratorPromptAssembler.php), S-4.1.1): it reads the registry for `agent IN (narrator, both)`, folds each lit block (`POV_CONTRACT`/`BEAT`/`LOREBOOK`/`SCENE_STATE`/`RESUME_ANCHOR`) from its producer, and **skips blocks with no producer yet** (`MESH_AWARENESS`/`DIRECTOR_STATE`, Phase 4) with no filler — see [Narrator prompt assembly](Diagrams/Engine/Narrator_Prompt_Assembly.md). The NPC half is built in Phase 2 and shares the registry-driven selection. The **prose call** that sends this assembled prompt and routes its structured `Handoff` into the spine is now built too ([`NarratorTurnService`](../../app/Services/Narrator/NarratorTurnService.php), S-4.2.1/S-4.2.2 — see [Narrator turn](Diagrams/Engine/Narrator_Turn_Prose_Call.md)). Every LLM call here goes through the [OpenRouter `LlmClient`](../adr/0017-llm-orchestration-openrouter.md) (ADR 0017), routed by **role** (`narrator_prose`, `recorder`, `npc_major`, `npc_minor`, `compiler`, …) and logged to `llm_calls`.
...
### Sprint 12 — Minimal manual structure (E1.2 / S-1.2.1)

S-1.2.1 turns the **Structure** placeholder into a live authoring surface — the hand-authored **chapter → scene → beat** the loop plays through, with **no LLM call and no API key**. It is the fourth nested, scope-bound owned resource, built as a sibling of the character / lorebook / reveal-ledger surfaces (ADR 0015 minimal slice).

- **Structure CRUD (S-1.2.1).** A single `StructureController` owns the whole surface (index + chapter/scene/beat store/update/destroy) over a thin `StructureService` (atomic via `DB::transaction`; the per-parent `number` is `max(number) + 1` taken under `lockForUpdate()` so concurrent creates never collide on the `(parent, number)` unique index). The parent `{story:slug}` binds under `OwnerScope`; the nested `{chapter}`/`{scene}`/`{beat}` children use **`->scopeBindings()`** down the `{story}→{chapter}→{scene}→{beat}` chain, so a row from another story (or a mismatched parent) → 404 without a row-level policy — authorization stays on the parent `Story` (`view` to read, `update` to write). Validation via `Store`/`Update` `Chapter`/`Scene`/`Beat` requests (shared `Guards{Chapter,Scene,Beat}Fields` traits). Writes are throttled (30/min).
- **Scene POV contract.** A scene carries `pov_mode` (the `PovMode` enum), a `pov_anchor` (viewpoint character), an optional `tone`, and its `present_characters` cast. The anchor + present cast are stored as **character slugs** and validated at request time against the story's characters: every present slug must belong to the story and the anchor must be **both a story character and one of the present cast** (the viewpoint must be in the scene). `elapsed_source` has no DB default, so a hand-authored scene records `Default`/`Continuous` (declaring an in-world gap is deferred).
- **Beat goal only.** A beat's `goal` is its satisfaction anchor — the only authored field this phase. The full beat document (`intent`, `word_budget`, `nudge_target`) lands in Phase 4, so `intent`/`word_budget` are written as deferred defaults (`''` / `StructureService::DEFAULT_WORD_BUDGET`) to satisfy their `NOT NULL` columns without surfacing them (**PH-35**).
- **Chapter-delete guard.** Deleting a chapter cascades to its scenes/beats *and* its `character_cards`; the latter would orphan the E1.1 cast, so `StructureController` rejects the delete with a clear toast when the chapter still anchors character cards (Chapter 1 stays protected while characters exist). The frontend mirrors the guard (disabled delete + hint), but the server is the enforcement.
- **Workspace surface.** `resources/js/pages/stories/Structure.vue` (inside the per-story workspace layout) renders the chapter → scene → beat tree as nested cards with per-level **edit / delete** and **Add scene / Add beat** actions and a single primary **New chapter**; the three `useForm`-driven dialogs (`ChapterDialog` / `SceneDialog` / `BeatDialog`, under `components/stories/structure/`) submit via Wayfinder and reseed on open. Scenes need a viewpoint character, so an empty cast disables **Add scene** behind inline guidance linking to the Characters tab. Deletes go through `useConfirm`; feedback is server flash → `vue-sonner`. **Resolves the Structure portion of PH-30** (only Saves remains a placeholder).
- **Play-readiness.** Authoring a chapter → scene → beat flips the **Overview** structure requirement green (`StoryOverviewService::readiness()` already keys on `beats >= 1`); no readiness change was needed.
...
### Sprint 13 — Session fork & save management (E2.1 / S-2.1.1–S-2.1.3)

S-2.1.1 turns the **Saves** placeholder into the first **save-realm** surface: starting a session **forks** a play-ready story into a save that evolves without touching the template. It is the first producer of `play_sessions` rows — `play_sessions` was migrated behaviorless in Phase 0. S-2.1.2/S-2.1.3 then make saves first-class: name/rename/reset/delete independent saves, and resume each at its persisted loop position.
...
- **Multi-save management (S-2.1.2).** `SessionService` grows `rename()`, `reset()`, and `delete()`; `SessionController` adds `update`/`reset`/`destroy` (all `update`-gated, scoped-bound, throttled 30/min). The fork accepts an optional author **name** (`StartSessionRequest`, falling back to `Playthrough N`); rename takes a required name (`RenameSessionRequest`). **Reset** re-uses the fork's first-beat resolution and, in a `DB::transaction`, returns the save to `session_start` with every loop-state counter cleared (`beat_word_count`/`chapter_word_count = 0`, `nudge_level`/`resume_anchor`/`narrative_clock = null`) — keeping the same id/name, and re-stamping `last_played_at`. Each save is **independent**: rename/reset/delete touch only the addressed row, never a sibling or the authoring template. `Saves.vue` replaces the bare Start button with a `SaveDialog` (create + rename modes) and gives each save card explicit **Open / Rename / Reset / Delete** actions; reset and delete are confirmed via `useConfirm` (never a native dialog).
- **Loop-state resume (S-2.1.3).** Opening a save *is* resuming it: `play()` calls `SessionService::resume()` to stamp `last_played_at` (so the list sorts most-recent and "continue where I left off" is accurate) and renders the save at its **persisted** `state_node` + position — never reset to the beat start. `presentSave()` now exposes `resumeAnchor`, letting `Play.vue` show "resuming" vs "starting fresh". The columns are persisted/restored now, but the producers that **advance** them mid-play — the state machine spine (S-3.1.1), the resume anchor (S-5.3.1), and the word/nudge clocks (Phase 4) — are later (**PH-37**).
...
- **Deferred.** The handoff producer (S-4.2.1 — **now built**, Sprint 15); player input commit (S-5.1.1 — **now built**, Sprint 16); the Play advance UI (S-5.4.1 — **now built**, Sprint 16); the resume anchor (S-5.3.1); the `npc_moment` branch + recorder-first sequencing (Phase 2); boundary subsystems (Phase 4).
...
### Sprint 15 — Narrator prose call (E4.2 / S-4.2.1 / S-4.2.2)

The **handoff producer** PH-37 reserved: one narrator turn that runs a structured prose call and feeds its validated `Handoff` into the spine (S-3.1.1). Built backend + tests first; its reachable advance control + prose reader then shipped with the Play reader (S-5.4.1, Sprint 16). S-4.2.1 is the success path; S-4.2.2 is the safety guarantee — malformed output is retried then surfaced, never trusted.

- **The turn (S-4.2.1).** `NarratorTurnService` ([`app/Services/Narrator/NarratorTurnService.php`](../../app/Services/Narrator/NarratorTurnService.php)) assembles the narrator prompt (S-4.1.1), resolves the `narrator_prose` model, and runs **one** `completeStructured` call against [`NarratorProseSchema`](../../app/Services/Narrator/NarratorProseSchema.php) (`prose · handoff · elapsed_bucket`), returning a typed [`NarratorTurnResult`](../../app/Services/Narrator/NarratorTurnResult.php). Handoff detection is the prose call's **own output**, not a separate classifier (ADR 0016 §4).
- **Call before transition (S-4.2.2).** The LLM call runs **before** any spine mutation, so a malformed/failed call surfaces without advancing the loop. Only a validated result advances: `begin` if the save is at `session_start`, then `applyHandoff(handoff)`. The schema's `handoff` enum is narrowed to `player_moment | beat_complete` this phase (`npc_moment` excluded until Phase 2).
- **Validator hardening (S-4.2.2 core).** `OpenRouterClient::conformsToSchema()` now validates declared `enum` membership in addition to required keys + types — an out-of-vocabulary value is "malformed", retried to the bound, recorded `Failed`, and surfaced as `LlmStructuredOutputException`. Guarded so enum-less schemas are unaffected.
- **Surfacing.** `SessionController@narrate` catches `IllegalLoopTransitionException` / `UnresolvedModelRoleException` / `LlmCallFailedException` and flashes an error toast (state untouched, prior position loadable); a validated turn flashes a success toast. Both redirect to `stories.saves.play`.
- **Deferred.** `elapsed_bucket` consumption by decay (Phase 5, PH-40); the resume anchor (S-5.3.1); the recorder sub-call + `npc_moment` (Phase 2). (Persisting prose/handoff into the `events` scene log and the reachable advance control + prose reader shipped in Sprint 16, S-5.2.1/S-5.4.1.)

Route: `POST stories.saves.narrate` (`throttle:30,1`, `scopeBindings`, `update` gate). Contract: [../api/saves.md](../api/saves.md). Tests: [`tests/Feature/Narrator/NarratorTurnTest.php`](../../tests/Feature/Narrator/NarratorTurnTest.php) + the enum case in [`tests/Feature/Llm/StructuredOutputRetryTest.php`](../../tests/Feature/Llm/StructuredOutputRetryTest.php). Diagram: [Diagrams/Engine/Narrator_Turn_Prose_Call.md](./Diagrams/Engine/Narrator_Turn_Prose_Call.md).

### Sprint 16 — Writing/Play reader + scene log + player moment (E5 / S-5.4.1 / S-5.2.1 / S-5.1.1)

The host the loop plays in, now filled: the reshaped `sessions/Play.vue` renders the save's prose scrollback, the scene log persists every turn, and the player writes back at a player moment — closing the narrator → me → narrator loop end-to-end from the UI. The two-layer record + sourced delivery stay deferred to Phase 2; this slice keeps a single canonical prose history.

- **Scene log persistence (S-5.2.1, partial).** `SceneLogService` ([`app/Services/SceneLogService.php`](../../app/Services/SceneLogService.php)) appends immutable `events` rows — `recordNarration` (prose · handoff · token estimate) on a narrator turn and `recordPlayerInput` (the player's prose) at a player moment — so the scrollback and later turns can read what came before. The bounded ~2000-token window + scene-summary compaction at SCENE_DONE are still later in E5.2; this slice records the raw entries. **Resolves the prose-persistence half of PH-40** (`elapsed_bucket` consumption stays Phase 5).
- **Player moment (S-5.1.1).** `SessionService::recordPlayerMoment()` hands the turn back to the narrator (`resumeFromPlayerMoment`) and appends the input to the scene log inside **one `DB::transaction`**, so a mid-turn failure rolls the whole moment back (the save stays on `player_moment` with nothing recorded). `SessionController@input` validates via `SubmitPlayerInputRequest` (content required, max 5000) and delegates; acting off-turn is surfaced as an error toast with the save unchanged. The human supplies the behaviour — `is_player` is never simulated. Diagram: [Diagrams/Engine/Player_Moment_Flow.md](./Diagrams/Engine/Player_Moment_Flow.md).
- **The Writing/Play reader (S-5.4.1).** `sessions/Play.vue` is now the Novel-Crafter-style workspace: a codex rail (position · cast · world, read-only — never feeds a prompt), a serif `max-w-prose` scrollback that reads the scene log, and a **single turn control** that adapts to `flow` — the narrator advances ("Begin the scene" / "Continue"), the player writes back (autofocused composer + ⌘/Ctrl+Enter + a character counter toward the 5000 cap), the player continues past a beat boundary, or the story has ended. A failed generation keeps prior prose readable and lets the player retry without losing the save. **Resolves PH-36 + PH-41.**
- **Flow derivation.** `SessionController@play` exposes a `flow` prop derived from `state_node` (+ a next-beat lookahead via `BeatSequence`) so exactly one control shows: `awaitingNarrator` | `awaitingPlayer` | `atBeatBoundary` | `ended`.
- **Deferred.** The bounded immediate-context window + scene summary (S-5.2.1 remainder); the resume anchor content (S-5.3.1); `elapsed_bucket` decay (Phase 5); the two-layer record + sourced delivery + NPC turns (Phase 2).

Routes: `POST stories.saves.input` + `POST stories.saves.continue` (both `throttle:30,1`, `scopeBindings`, `update` gate). Contract: [../api/saves.md](../api/saves.md). Tests: [`tests/Feature/Sessions/PlayLoopTest.php`](../../tests/Feature/Sessions/PlayLoopTest.php). Diagram: [Diagrams/Engine/Player_Moment_Flow.md](./Diagrams/Engine/Player_Moment_Flow.md).
