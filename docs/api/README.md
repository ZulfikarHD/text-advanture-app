...
>
> **Sprint 13** added the per-story **session fork** ([saves.md](./saves.md), E2.1 / S-2.1.1) — the first **save-realm** surface: starting a session atomically forks a play-ready story into one `play_sessions` row at `session_start`, positioned at its first beat, without mutating the authoring template (ADR 0012) and seeding no edges yet (Phase 5). **Resolves PH-30** (Saves was the last `ComingSoon` surface; the shared placeholder is removed). It also shipped multi-save management (S-2.1.2) and resume (S-2.1.3).
>
> **E3.1 / E4.1 (engine internals) added no HTTP endpoints.** The state-machine spine (S-3.1.1, `SessionStateMachine`) and the **narrator prompt assembly** (S-4.1.1, `NarratorPromptAssembler` — reads the `prompt_blocks` registry to fold the lit narrator blocks into the turn's prompt) are backend services with **no route**: the assembler produces chat messages, and the prose call that *sends* them (S-4.2.1). See [features/narrator/S-4.1.1-narrator-prompt-assembly.md](../features/narrator/S-4.1.1-narrator-prompt-assembly.md).
>
> **Sprint 16** filled the **Writing/Play loop** ([saves.md](./saves.md), E5 / S-5.4.1 / S-5.1.1) — the Play page now `POST`s `narrate` (S-4.2.1), `input` (the player moment, S-5.1.1 — `SessionService::recordPlayerMoment()` records the input + hands the turn back atomically), and `continue` (beat boundary, S-3.1.2), reading the save's `events` scene log into a prose scrollback. **Resolves PH-36 + PH-41.**
