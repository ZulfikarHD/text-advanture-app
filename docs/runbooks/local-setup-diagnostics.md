...
- **Reach play-ready by hand (E1.1 + E1.2).** A fresh story shows **not play-ready** on its **Overview** tab. To turn every requirement green with no engine: (1) seed the model roles — `php artisan db:seed --class=GlobalLibrarySeeder` (the *model* role requirement), (2) open the story → **Characters** → add at least one character (the first one auto-creates **Chapter 1**), then (3) **Structure** → **New chapter** (or reuse Chapter 1) → **Add scene** (needs ≥ 1 character to anchor the POV) → **Add beat** (a `goal`). One beat satisfies the structure requirement (`StoryOverviewService::readiness()` keys on `beats >= 1`). If **Add scene** is disabled, the cast is empty — add a character first (the surface links you there). If a **chapter delete** is refused with "anchors character cards", that chapter still holds the cast's chapter-1 cards (Chapter 1 while characters exist) — move/delete those characters first.
