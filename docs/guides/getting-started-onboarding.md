...
Auth surfaces are the only public pages; everything else is behind the `auth` middleware. After signing in you land on `/dashboard` (the **Workspace**) — or your intended destination if you were redirected from a protected page. The sidebar surfaces **Workspace** + **Settings**; play is **chapter-centric**, reached per-book (open a story → **Saves** → Start/Open, or a chapter), not a global sidebar item (PH-15).
...
## 6. What exists today (Phase 1–2, through E2.1 — session fork)

The app boots, authenticates, isolates accounts, themes (light/dark/system, with a quick toggle in the user menu), and navigates the **Workspace + Settings** shell — but has **no story-authoring UI yet**. Sprint 2 added the account-isolation foundation, the registration toggle, and the shell; Sprint 3 added the theming/accessibility polish (skip-link, tokenized `Welcome`), reusable four-state components (`EmptyState`/`ErrorState`) plus a promise-based `useConfirm()` for destructive actions, and stood up the **authoring-realm schema** — 11 tables with enums, models, and factories, with `stories` as the first owner-scoped model. Sprint 4 completed the **persistence engine**: the 5 global libraries and 16 save-realm tables (the save root is `play_sessions`), with append-only audit tables guarded by the `AppendOnly` trait and `beat_true_states` kept structurally isolated from `beat_records.surface`; it also resolved the deferred FKs (PH-16) and shipped the first **Settings → Provider** screen for storing a per-account **encrypted** LLM key (`provider_credentials`). Sprint 5 built the **LLM client** behind the provider-agnostic `App\Contracts\Llm\LlmClient` interface (`OpenRouterClient`, thin `Http` wrapper with retry/backoff + structured-output validation; `laravel/ai` was removed) and three settings surfaces: a **connection test** on the Provider page (`useHttp`), a **Model roles** editor (global role→model mapping), and an owner-scoped **Usage** log (deferred prop, USD cost, WIB time) — but there is no engine caller yet. Sprint 6 **seeded the five global libraries** (universal priors, register + character archetypes, the 16-row prompt-block registry, and the 8 default `model_profiles` — so Model roles is no longer empty) via the idempotent `GlobalLibrarySeeder`, and built the **review-gate foundation**: a `ReviewGateService` state machine (propose → accept/edit/reject, recording who/when) behind a new top-level **Review** (`/reviews`) surface, owner-scoped by a nullable `review_items.user_id`. The gate's per-producer commit handlers are deferred to Phase 7 (PH-27), so today it shows an empty teaching state. **Sprint 7 (Phase 2 kickoff)** delivers the first **authoring CRUD**: the workspace dashboard now lists the owner's stories as a card grid, with **create** (Dialog from dashboard), **edit** (dedicated `/stories/{slug}/edit` page), and **delete** (useConfirm, FK cascade). The `stories.slug` constraint was relaxed from global unique to per-owner unique (`(user_id, slug)`), and slug derivation from title with auto-suffix on collision is centralised in `StoryService`. **E1.2** then wraps each story in a **workspace sub-nav** (**Overview · Details · Settings**, reached by opening a story card): **Overview** shows derived authoring counts + a play-readiness gate (both computed on read), and **Settings** edits the story's default POV (`stories.settings.default_pov`) and per-role model overrides (`model_profiles` scope=`story`, preferred over the global defaults). **E2.1** then grows that sub-nav into the **full authoring surface set** — **Overview · Characters · Structure · Lorebook · Settings · Saves · Details**; the not-yet-built surfaces (Characters/Structure/Saves) render a reachable **`stories/ComingSoon`** teaching placeholder (`StoryPlaceholderController`) so the workspace shows its full shape without dead nav items, each repointed at its real controller when its feature ships (PH-30). **E3.1 (S-3.1.1)** ships the first of those — the **Lorebook** tab is now live per-story CRUD (keyword-injected world facts via `LorebookController` + `LorebookService`, the keyword chip input, and an owner/scope-bound nested resource); runtime injection and world-fact-discipline validation stay deferred (PH-31/PH-32). **E4.1** added the sibling **Reveal ledger** tab (load-bearing secrets → reveal point). **E1.1 (S-1.1.1/S-1.1.2)** then shipped the **Characters** tab — the first **minimal manual** creation surface (no LLM call): open a story → **Characters** tab → **New character**, where you mark exactly one **player** (appearance only) and give every other character (NPC) a folded identity and a mandatory knowledge boundary. Because a character's card is anchored to a chapter, the first character auto-creates a default **Chapter 1** (characters are tied to chapters); edges/registers and the AI creation pipeline stay deferred to Phase 5 (PH-30 resolved for characters). **E1.2 (S-1.2.1)** then ships the **Structure** tab — the hand-authored **chapter → scene → beat** the loop plays through (no LLM call): open a story → **Structure** tab → **New chapter**, then add a **scene** (its POV + present cast, anchored on a character) and a **beat** (its goal). Authoring one beat makes the story **play-ready** (the Overview structure requirement). The beat document (`intent`/`word_budget`) and outline compilation stay deferred to Phase 4 (PH-35; Structure portion of PH-30 resolved — only **Saves** was still a placeholder). **E2.1 (S-2.1.1)** then ships the **Saves** tab — start a playthrough: open a play-ready story → **Saves** tab → **Start session**, which **forks** the story into a save (a `play_sessions` row at `session_start`, positioned at the first beat) that evolves without touching the template, and lands you on the **Writing/Play** page (the reader + loop controls shipped in Sprint 16, S-5.4.1). **E2.1 (S-2.1.2/S-2.1.3)** then makes saves first-class: **name** a save on create, **rename** it, **reset** it back to its freshly-forked starting position, and **delete** it (both confirmed via `useConfirm`) — each save independent, so managing one never touches a sibling or the template; and **opening** a save resumes it at its **persisted** loop position (stamping `last_played_at`, so the list sorts most-recent), rather than restarting. The loop-state columns are persisted/restored now, but the producers that *advance* a save mid-play (state machine S-3.1.1, resume anchor S-5.3.1, word/nudge clocks Phase 4) are later (PH-37). With Saves live, **every workspace surface now exists** — `StoryPlaceholderController` and `stories/ComingSoon.vue` are removed (PH-30 resolved). The first **engine internals** are then built backend-only (no UI yet): **S-3.1.1** the deterministic state-machine spine (`SessionStateMachine`) that advances a save through `session_start → narrator_turn → { player_moment | beat_complete } → narrator_resumes`, and **S-4.1.1** the **narrator prompt assembly** (`App\Services\Narrator\NarratorPromptAssembler`) that reads the seeded `prompt_blocks` registry to fold the lit narrator blocks (`POV_CONTRACT`/`BEAT`/`LOREBOOK`/`SCENE_STATE`/`RESUME_ANCHOR`) into the turn's prompt — blocks with no producer yet (`MESH_AWARENESS`/`DIRECTOR_STATE`) are absent, with no filler. **S-4.2.1/S-4.2.2** then built the **prose call** that *sends* this prompt: `App\Services\Narrator\NarratorTurnService` runs one structured call (`prose · handoff · elapsed_bucket`) and feeds the validated `Handoff` into the spine — and the LLM client now validates `enum` membership, so malformed output (e.g. an out-of-vocabulary handoff) is retried then surfaced as a failed turn, never trusted (the loop never advances on bad output). It is backend + tests first, surfacing success/failure via toast. **Sprint 16 (E5)** then filled the **Writing/Play page** (`sessions/Play.vue`, S-5.4.1): a serif prose scrollback that reads the save's `events` scene log (persisted by `SceneLogService`, S-5.2.1), a read-only codex rail, and a single turn control that adapts to whose turn it is — the narrator advances, the **player writes back at a player moment** (S-5.1.1: `SessionService::recordPlayerMoment()` hands the turn back and records the input atomically), or the player continues past a beat. This closes the **narrator → me → narrator** loop end-to-end from the UI (see §7). The rest of the narrative engine (NPC turns, delta engine, full review commit) is designed in the [ADRs](../adr/README.md) and built across Phases 2–7. To understand the destination, read [ARCHITECTURE.md](../architecture/ARCHITECTURE.md).

## 7. Playing the loop (try it)

Once a story is **play-ready** (≥1 character incl. the player, ≥1 chapter/scene/beat) and a `narrator_prose` model + provider key are set (Settings → Model roles / Provider):

1. Open the story → **Saves** tab → **Start session** (the fork is invisible — you land straight on the Writing/Play page).
2. Click **Begin the scene** — the narrator writes the opening prose, then hands off.
3. At a **player moment**, the composer is autofocused: write your character's action/dialogue and press **⌘/Ctrl + Enter**. Your input joins the scrollback and the narrator continues from it (it never restarts the beat).
4. At a **beat boundary**, click **Continue to the next beat**; at the end, you get a clear way back to the book.

A failed narrator call surfaces a toast and leaves the save unchanged — retry without losing your place. The loop spine + flow live in [`SessionStateMachine`](../../app/Services/SessionStateMachine.php) / [`SessionService`](../../app/Services/SessionService.php); see the [player moment flow](../architecture/Diagrams/Engine/Player_Moment_Flow.md) and [narrator turn](../architecture/Diagrams/Engine/Narrator_Turn_Prose_Call.md) diagrams.
